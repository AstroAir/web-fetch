...
class LoggingConfig(BaseModel):
    ...
    enable_structured: bool = Field(default=False, description="Enable structured JSON logging")
...
class SecurityConfig(BaseModel):
    ...
    ssl_cert_path: Optional[Path] = Field(default=None, description="Custom SSL certificate path")
    ssl_key_path: Optional[Path] = Field(default=None, description="Custom SSL key path")
    ca_bundle_path: Optional[Path] = Field(default=None, description="Custom CA bundle path")
    
    # Request security
    max_redirects: int = Field(default=10, description="Maximum number of redirects to follow")
    max_response_size: int = Field(default=100 * 1024 * 1024, description="Maximum response size in bytes")
    ...
    allowed_hosts: Optional[List[str]] = Field(default=None, description="Allowed hostnames (whitelist)")
    
    # Authentication security
    mask_credentials: bool = Field(default=True, description="Mask credentials in logs")
    credential_cache_ttl: int = Field(default=3600, description="Credential cache TTL in seconds")
...
class PerformanceConfig(BaseModel):
    ...
    total_timeout: float = Field(default=60.0, description="Total request timeout in seconds")
    
    # Concurrency settings
    max_concurrent_requests: int = Field(default=50, description="Maximum concurrent requests")
    semaphore_timeout: float = Field(default=30.0, description="Semaphore acquisition timeout")
    ...
    # Memory settings
    chunk_size: int = Field(default=8192, description="Default chunk size for streaming")
    buffer_size: int = Field(default=64 * 1024, description="Buffer size for I/O operations")
...
class FeatureFlags(BaseModel):
    ...
    enable_deduplication: bool = Field(default=True, description="Enable request deduplication")
    ...
    # Content processing
    enable_content_parsing: bool = Field(default=True, description="Enable content parsing")
    ...
    enable_js_rendering: bool = Field(default=False, description="Enable JavaScript rendering")
    
    # External integrations
    enable_crawlers: bool = Field(default=True, description="Enable crawler integrations")
...
class EnvironmentConfig(BaseModel):
    ...
    @validator('data_dir', 'cache_dir', 'log_dir', 'temp_dir', pre=True)
