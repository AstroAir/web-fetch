...
from .models import GlobalConfig, LogLevel, Environment
...
class ConfigValidator:
    ...
    def _validate_performance(self, performance_config) -> None:
        ...
        if performance_config.max_connections_per_host > performance_config.max_connections:
            self.warnings.append("Max connections per host exceeds total max connections")
    ...
    def _validate_features(self, features_config) -> None:
        ...
        if not features_config.enable_caching and features_config.enable_deduplication:
            self.warnings.append("Deduplication without caching may have limited effectiveness")
        
        if features_config.enable_js_rendering:
            self.warnings.append("JavaScript rendering is experimental and may impact performance")
        ...
        for name, directory in directories:
            try:
                ...
                if dir_path.exists() and not dir_path.is_dir():
                    self.errors.append(f"{name} exists but is not a directory: {dir_path}")
            except Exception as e:
