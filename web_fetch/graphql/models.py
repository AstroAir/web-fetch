...
class GraphQLQuery:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"query": self.query, "variables": self.variables}
...
class GraphQLMutation(GraphQLQuery):
    ...
    operation_type: GraphQLOperationType = field(
        default=GraphQLOperationType.MUTATION, init=False
    )
...
class GraphQLSubscription(GraphQLQuery):
    ...
    operation_type: GraphQLOperationType = field(
        default=GraphQLOperationType.SUBSCRIPTION, init=False
    )
...
class GraphQLConfig(BaseModel):
    ...
    subscription_endpoint: Optional[HttpUrl] = Field(
        default=None, description="WebSocket endpoint for subscriptions"
    )
    ...
    introspection_enabled: bool = Field(
        default=True, description="Enable schema introspection"
    )
    schema_cache_ttl: int = Field(
        default=3600, ge=0, description="Schema cache TTL in seconds"
    )

    # Validation settings
    validate_queries: bool = Field(
        default=True, description="Validate queries before sending"
    )
    validate_variables: bool = Field(
        default=True, description="Validate variables against schema"
    )

    # Error handling
    raise_on_errors: bool = Field(
        default=False, description="Raise exception on GraphQL errors"
    )
    include_extensions: bool = Field(
        default=True, description="Include extensions in response"
    )

    # Performance settings
    enable_query_batching: bool = Field(
        default=False, description="Enable query batching"
    )
    max_batch_size: int = Field(
        default=10, ge=1, description="Maximum queries per batch"
    )
    ...
    cache_ttl: int = Field(
        default=300, ge=0, description="Response cache TTL in seconds"
    )
