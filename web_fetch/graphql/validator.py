...
class GraphQLSyntaxValidator:
    ...
    OPERATION_PATTERN = re.compile(r'^\s*(query|mutation|subscription)\s+', re.IGNORECASE)
    ...
    def _validate_balanced_braces(self, query_text: str, result: ValidationResult) -> None:
    ...
    def _validate_balanced_parentheses(self, query_text: str, result: ValidationResult) -> None:
        ...
        if paren_count > 0:
            ...
            result.add_error(f"Extra {abs(paren_count)} closing parenthesis/parentheses")
    ...
    def _validate_variable_syntax(self, query_text: str, result: ValidationResult) -> None:
...
class GraphQLSchemaValidator:
    ...
    def _build_field_map(self, fields: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    def validate_against_schema(
        ...
        for field_name in query_fields:
            if field_name not in available_fields:
                result.add_error(f"Field '{field_name}' does not exist in {operation_type.value} type")
    ...
    def _extract_fields_from_query(self, query_text: str) -> Set[str]:
        ...
        for line in lines:
            ...
            if in_selection:
                ...
                field_matches = re.findall(r'\b([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:\(|{|$)', stripped)
...
class GraphQLValidator:
    ...
    def _validate_complexity(
        ...
        if depth > self.max_query_depth:
            result.add_error(f"Query depth {depth} exceeds limit {self.max_query_depth}")
        ...
        if complexity > self.max_query_complexity:
            result.add_error(f"Query complexity {complexity} exceeds limit {self.max_query_complexity}")
