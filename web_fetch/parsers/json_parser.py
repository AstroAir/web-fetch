...
class JSONParser:
    ...
    def _detect_api_standard(
        self, data: Dict[str, Any], headers: Optional[Dict[str, str]]
    ) -> Optional[str]:
    ...
    def _parse_json_ld(
        self, data: Dict[str, Any], url: Optional[str]
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    def _has_nested_objects(self, obj: Any) -> bool:
        ...
        if isinstance(obj, dict):
            return any(
                isinstance(v, dict) or self._has_nested_objects(v) for v in obj.values()
            )
        elif isinstance(obj, list):
