...
from ..models.base import FeedItem, FeedMetadata
...
class FeedParser:
    ...
    def __init__(self):
        ...
        if not HAS_FEEDPARSER:
            raise ImportError(
                "feedparser is required for feed parsing. Install with: pip install feedparser"
            )
    ...
    def parse(
        ...
        try:
            ...
            feed_data = {
                ...
                "last_updated": (
                    feed_metadata.last_build_date.isoformat()
                    if feed_metadata.last_build_date
                    else None
                ),
                "items": [
                    self._item_to_dict(item) for item in feed_items[:10]
                ],  # Include first 10 items
            }
    ...
    def _extract_feed_metadata(self, parsed_feed, url: Optional[str]) -> FeedMetadata:
        ...
        metadata.description = getattr(feed_info, "description", None) or getattr(
        ...
        metadata.managing_editor = getattr(
            feed_info, "managingEditor", None
        ) or getattr(feed_info, "author", None)
        ...
        metadata.feed_type = (
            parsed_feed.version.split()[0] if parsed_feed.version else "Unknown"
        )
    ...
    def _extract_feed_items(self, parsed_feed, url: Optional[str]) -> List[FeedItem]:
        ...
        for entry in parsed_feed.entries:
            ...
            item.description = getattr(entry, "description", None) or getattr(
                entry, "summary", None
            )
    ...
    def is_valid_feed(self, content: bytes) -> bool:
        ...
        try:
            ...
            return (
                hasattr(parsed_feed, "feed")
                and (
                    hasattr(parsed_feed.feed, "title")
                    or hasattr(parsed_feed.feed, "description")
                )
                and len(parsed_feed.entries) > 0
            )
        except Exception:
            return False
