...
import json
import logging
from typing import Union, Dict, Any, Optional, Tuple, List, TYPE_CHECKING, Literal, TypeAlias
...
class EnhancedContentParser:
    ...
    def link_extractor(self) -> 'LinkExtractor':
        ...
        if not hasattr(self, '_link_extractor') or self._link_extractor is None:
            from .link_extractor import LinkExtractor
            self._link_extractor = LinkExtractor(validate_links=False)  # Don't validate by default for performance
        ...
        try:
            match requested_type:
                ...
                case ContentType.TEXT:
                    ...
                    if isinstance(content, str) and len(content) > 100:  # Only analyze substantial text
                        try:
                            result.content_summary = self.content_analyzer.analyze_content(content)
                ...
                case ContentType.HTML:
                    ...
                    if isinstance(content, dict) and 'text' in content and len(content['text']) > 100:
                        try:
                            result.content_summary = self.content_analyzer.analyze_content(content['text'])
                        except Exception as e:
                            logger.warning(f"Content analysis failed: {e}")
    ...
    def _parse_json(self, content_bytes: bytes, url: Optional[str] = None, headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'pagination': self.json_parser.extract_pagination_info(parsed_data) if isinstance(parsed_data, dict) else None,
            }
        except Exception as e:
    ...
    async def _parse_image(
        ...
        try:
            image_data, image_metadata = self.image_parser.parse(content_bytes, url, headers)
    ...
    async def _parse_feed(
        ...
        try:
            feed_data, feed_metadata, feed_items = self.feed_parser.parse(content_bytes, url)
    ...
    async def _parse_markdown(
        ...
        try:
            ...
            # Convert to Markdown
            markdown_content = self.markdown_converter.convert(html_content['raw_html'], url)
