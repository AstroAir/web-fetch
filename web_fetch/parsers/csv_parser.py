...
class CSVParser:
    ...
    def parse(
        ...
        try:
            ...
            # Parse CSV data
            if self.use_pandas:
                data, metadata = self._parse_with_pandas(text_content, dialect, encoding)
            else:
    ...
    def _parse_with_csv(
        ...
        for i, col in enumerate(headers):
            null_count = sum(1 for row in data_rows if i >= len(row) or not row[i].strip())
    ...
    def _detect_header(self, rows: List[List[str]]) -> bool:
        ...
        for i, (first_val, second_val) in enumerate(zip(first_row, second_row)):
            ...
            if any(word in first_val.lower() for word in ['id', 'name', 'date', 'time', 'count', 'value']):
                header_score += 1
    ...
    def _infer_column_types(self, data_rows: List[List[str]], headers: List[str]) -> Dict[str, str]:
        ...
        for i, header in enumerate(headers):
            ...
            elif all(val.lower() in ['true', 'false', '1', '0', 'yes', 'no'] for val in sample_values):
    ...
    def get_csv_info(self, content: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'has_header': self._detect_header([first_row, next(reader, [])]) if first_row else False
            }
