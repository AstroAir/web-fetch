    ...
    def _extract_metadata(self, pdf_reader: pypdf.PdfReader) -> PDFMetadata:
        ...
        try:
            if pdf_reader.metadata:
                ...
                metadata.title = self._clean_metadata_value(
                    pdf_reader.metadata.get("/Title")
                )
                metadata.author = self._clean_metadata_value(
                    pdf_reader.metadata.get("/Author")
                )
                metadata.subject = self._clean_metadata_value(
                    pdf_reader.metadata.get("/Subject")
                )
                metadata.creator = self._clean_metadata_value(
                    pdf_reader.metadata.get("/Creator")
                )
                metadata.producer = self._clean_metadata_value(
    ...
    def get_page_text(self, content: bytes, page_number: int) -> str:
        ...
        try:
            ...
            if page_number < 1 or page_number > len(pdf_reader.pages):
                raise ContentError(
                    f"Page {page_number} does not exist. PDF has {len(pdf_reader.pages)} pages."
                )
