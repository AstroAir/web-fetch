...
from .base import CrawlerConfig, CrawlerType
...
class CrawlerManagerConfig:
    ...
    # Fallback order
    fallback_crawlers: list[CrawlerType] = field(
    ...
    def set_crawler_config(
        self, crawler_type: CrawlerType, config: CrawlerAPIConfig
    ) -> None:
...
class ConfigManager:
    ...
    def _load_from_environment(self) -> None:
        ...
        self._config.enable_fallback = (
            os.getenv("WEB_FETCH_ENABLE_FALLBACK", "true").lower() == "true"
        )
        self._config.log_api_calls = (
            os.getenv("WEB_FETCH_LOG_API_CALLS", "false").lower() == "false"
            and False
            or os.getenv("WEB_FETCH_LOG_API_CALLS", "false").lower() == "true"
        )
        self._config.cache_results = (
            os.getenv("WEB_FETCH_CACHE_RESULTS", "false").lower() == "true"
        )

        try:
            self._config.cache_ttl_seconds = int(
                os.getenv("WEB_FETCH_CACHE_TTL", "3600")
            )
        except ValueError:
            pass  # Keep default
