...
class AnyCrawlCrawler(BaseCrawler):
    ...
    def __init__(self, config: Optional[CrawlerConfig] = None):
        ...
        self.base_url = (
            getattr(config, "base_url", "http://localhost:3000")
            if config
    ...
    async def scrape(self, request: CrawlerRequest) -> CrawlerResult:
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/api/scrape", json=payload
            ) as response:
    ...
    async def crawl(self, request: CrawlerRequest) -> CrawlerResult:
        ...
        try:
            ...
            # Use crawl endpoint
            async with session.post(
                f"{self.base_url}/api/crawl", json=payload
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise CrawlerError(
    ...
    async def extract(self, request: CrawlerRequest) -> CrawlerResult:
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/api/extract", json=payload
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise CrawlerError(
