...
class BatchManager:
    ...
    async def submit_batch(self, batch_request: BatchRequest) -> str:
        ...
        logger.info(
            f"Submitted batch {batch_id} with {len(batch_request.requests)} requests"
        )
        return batch_id
    ...
    def list_batches(self, status: Optional[BatchStatus] = None) -> List[str]:
        ...
        return [
            batch_id
            for batch_id, result in self._results.items()
    ...
    async def _manager_loop(self) -> None:
        ...
        try:
            while not self._shutdown:
                ...
                if (
                    next_batch
                    and len(self._running_batches) < self.config.max_concurrent_batches
                ):
    ...
    async def _process_batch(self, batch_request: BatchRequest) -> None:
        ...
        try:
            ...
            result.completed_at = time.time()
            result.total_time = result.completed_at - (
                result.started_at or result.completed_at
            )
            ...
            if batch_request.completion_callback:
                try:
                    ...
                    logger.error(
                        f"Error in completion callback for batch {batch_id}: {e}"
                    )

            logger.info(
                f"Completed batch {batch_id} with {result.success_rate:.1%} success rate"
            )
            ...
            if batch_request.error_callback:
                try:
                    ...
                    logger.error(
                        f"Error in error callback for batch {batch_id}: {callback_error}"
                    )
    ...
    def _update_metrics(self) -> None:
        ...
        completed = sum(
            1 for r in self._results.values() if r.status == BatchStatus.COMPLETED
        )
        failed = sum(
            1 for r in self._results.values() if r.status == BatchStatus.FAILED
        )
