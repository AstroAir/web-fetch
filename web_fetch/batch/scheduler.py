...
from .models import BatchRequest, BatchConfig, BatchPriority, BatchStatus
...
class BatchScheduler:
    ...
    def __init__(self, config: BatchConfig):
        ...
        """
        self.config = config
        self._queue: List[tuple] = []  # Priority queue: (priority, timestamp, batch_request)
        self._waiting_batches: Dict[str, BatchRequest] = {}  # Batches waiting for dependencies
    ...
    async def add_batch(self, batch_request: BatchRequest) -> None:
        ...
        async with self._lock:
            ...
            if batch_request.depends_on:
                ...
                logger.debug(f"Batch {batch_request.id} waiting for dependencies: {batch_request.depends_on}")
            else:
    ...
    async def get_queue_status(self) -> Dict[str, int]:
        ...
        async with self._lock:
            ...
            valid_queue_items = sum(1 for _, _, batch in self._queue if batch is not None)
    ...
    async def _add_to_queue(self, batch_request: BatchRequest) -> None:
        ...
        logger.debug(f"Added batch {batch_request.id} to queue with priority {batch_request.priority}")
    ...
    async def _check_waiting_batches(self) -> None:
        ...
        for batch_request in ready_batches:
            await self._add_to_queue(batch_request)
            logger.debug(f"Batch {batch_request.id} dependencies resolved, added to queue")
...
class FairScheduler(BatchScheduler):
    ...
    async def _add_to_queue(self, batch_request: BatchRequest) -> None:
        ...
        logger.debug(f"Added batch {batch_request.id} with aged priority {adjusted_priority}")
...
class ResourceAwareScheduler(BatchScheduler):
    ...
    def _can_schedule_batch(self, batch_request: BatchRequest) -> bool:
        ...
        memory_ok = self._resource_usage['memory'] + estimated_memory < self.config.memory_limit_mb
