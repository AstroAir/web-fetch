...
from .models import BatchRequest, BatchResult, BatchConfig, BatchStatus
...
class BatchProcessor:
    ...
    async def process_batch(
        ...
        try:
                ...
                for i, request in enumerate(batch_request.requests):
                    task = asyncio.create_task(
                        self._process_single_request(
                            fetcher, request, semaphore, batch_id, i,
            ...
            logger.info(f"Batch {batch_id} completed: {result.successful_requests}/{result.total_requests} successful")
        ...
        async with semaphore:
            ...
            try:
                ...
                if progress_callback:
                    progress_data = {
                        'completed': batch_result.successful_requests + batch_result.failed_requests,
                        'failed': batch_result.failed_requests,
                        'current_url': str(request.url) if hasattr(request, 'url') else None
                    }
                ...
                logger.error(f"Error in request {request_index} of batch {batch_id}: {e}")
        ...
        try:
            ...
            async with WebFetcher(fetch_config) as fetcher:
                ...
                async for request_result in self._stream_requests(
                    ...
                    if progress_callback:
                        progress_data = {
                            'completed': result.successful_requests + result.failed_requests,
                            'failed': result.failed_requests,
                            'current_url': str(request_result.url) if request_result.url else None
                        }
                        progress_callback(progress_data)
