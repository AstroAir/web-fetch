...
class BatchResult:
    ...
    def _update_timing_metrics(self, response_time: float) -> None:
        ...
        if self.successful_requests == 1:
            ...
            self.average_response_time = (old_avg * (self.successful_requests - 1) + response_time) / self.successful_requests
...
class BatchConfig(BaseModel):
    ...
    max_concurrent_batches: int = Field(default=5, description="Maximum concurrent batches")
    max_concurrent_requests_per_batch: int = Field(default=10, description="Max concurrent requests per batch")
    
    # Queue settings
    max_queue_size: int = Field(default=1000, description="Maximum queue size")
    priority_queue_enabled: bool = Field(default=True, description="Enable priority queue")
    
    # Retry settings
    max_batch_retries: int = Field(default=3, description="Maximum batch retry attempts")
    retry_delay: float = Field(default=5.0, description="Delay between batch retries")
    
    # Timeout settings
    batch_timeout: float = Field(default=3600.0, description="Maximum batch execution time")
    ...
    results_directory: Optional[str] = Field(default=None, description="Results directory")
