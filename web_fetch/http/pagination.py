...
class PaginationConfig(BaseModel):
    ...
    next_cursor_field: Optional[str] = Field(default="next_cursor", description="Next cursor field")
    has_more_field: Optional[str] = Field(default="has_more", description="Has more field")
    
    # Custom extraction functions
    custom_next_url_extractor: Optional[callable] = Field(default=None, description="Custom next URL extractor")
    custom_data_extractor: Optional[callable] = Field(default=None, description="Custom data extractor")
...
class PaginationResult(BaseModel):
    ...
    next_cursor: Optional[str] = Field(default=None, description="Next cursor for continuation")
    responses: List[FetchResult] = Field(default_factory=list, description="Individual page responses")
...
class PageSizeHandler(BasePaginationHandler):
    ...
    async def get_next_request(
        ...
        if current_response:
            ...
            if (isinstance(current_response.content, dict) and 
                self.config.has_more_field and 
                not current_response.content.get(self.config.has_more_field, True)):
                return None
...
class PaginationHandler:
    ...
    async def fetch_all_pages(
        ...
        if self.config.strategy not in self._handlers:
            raise WebFetchError(f"Unsupported pagination strategy: {self.config.strategy}")
        ...
        while page_number <= self.config.max_pages:
            # Get next request
            request = await handler.get_next_request(base_request, current_response, page_number)
            if not request:
                break
