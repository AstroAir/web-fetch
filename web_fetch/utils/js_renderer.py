...
class JSRenderConfig:
    ...
    block_resources: List[str] | None = None  # Resource types to block
    ...
    def __post_init__(self) -> None:
        ...
        if self.block_resources is None:
            self.block_resources = [
                ...
                "media",
                "font",
            ]  # Block by default for performance
...
class JavaScriptRenderer:
    ...
    def __init__(self, config: Optional[JSRenderConfig] = None):
        ...
        if not HAS_PLAYWRIGHT:
            raise ImportError(
                "playwright is required for JavaScript rendering. Install with: pip install playwright"
            )
    ...
    async def render_page(self, url: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            wait_for_selector = kwargs.get(
                "wait_for_selector", self.config.wait_for_selector
            )
            wait_for_function = kwargs.get(
                "wait_for_function", self.config.wait_for_function
            )
    ...
    async def _handle_route(self, route):
        ...
        if self.config.block_resources and resource_type in self.config.block_resources:
            await route.abort()
        else:
    ...
    async def install_browsers(self):
        ...
        try:
            ...
            logger.info(
                f"Successfully installed {self.config.browser_type.value} browser"
            )
