...
class AdvancedRateLimiter:
    ...
    async def acquire(
        ...
        if domain_info.circuit_breaker and domain_info.circuit_breaker.state == "open":
    ...
    async def record_response(
        ...
        if domain_info.circuit_breaker:
            if 200 <= status_code < 300:
                await domain_info.circuit_breaker._on_success()
                domain_info.consecutive_failures = 0
                domain_info.last_success = current_time
            elif status_code == 429 or status_code >= 500:
                # Create a dummy exception for circuit breaker
                from ..exceptions import HTTPError

                error = HTTPError(f"HTTP {status_code}", status_code)
                await domain_info.circuit_breaker._on_failure(error)
    ...
    def _create_domain_limit_info(self, domain: str) -> DomainLimitInfo:
        ...
        if self.config.circuit_breaker_enabled:
            ...
            domain_info.circuit_breaker = CircuitBreaker(domain, cb_config)

        return domain_info

    def _update_server_limits(
        self, domain_info: DomainLimitInfo, headers: Dict[str, str]
    ) -> None:
    ...
    async def _token_bucket_delay(self, domain_info: DomainLimitInfo) -> float:
        ...
        domain_info.tokens = min(
            self.config.burst_size, domain_info.tokens + tokens_to_add
        )
    ...
    async def _leaky_bucket_delay(self, domain_info: DomainLimitInfo) -> float:
        ...
        if current_level < self.config.burst_size:
            ...
            # Calculate delay
            delay = (
                current_level - self.config.burst_size + 1
            ) / domain_info.current_rps
            return delay
    ...
    async def _adapt_rate_limit(
        self, domain_info: DomainLimitInfo, status_code: int, response_time: float
    ) -> None:
        ...
        logger.debug(
            f"Adapted rate limit for {domain_info.domain}: {current_rps:.2f} -> {new_rps:.2f} RPS"
        )

    def _clean_old_requests(
        self, domain_info: DomainLimitInfo, current_time: float
    ) -> None:
        """Clean old request timestamps."""
        cutoff_time = (
            current_time - self.config.window_size * 2
        ...
        if domain:
            if domain in self.domain_limits:
                ...
                return {
                    ...
                    "circuit_breaker_state": (
                        domain_info.circuit_breaker.state.value
                        if domain_info.circuit_breaker
                        else None
                    ),
                }
            ...
            return {
                domain: {
                    ...
                    "circuit_breaker_state": (
                        info.circuit_breaker.state.value
