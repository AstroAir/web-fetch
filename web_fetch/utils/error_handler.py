...
from ..exceptions import AuthenticationError, RateLimitError, ServerError, WebFetchError
...
class EnhancedErrorHandler:
    ...
    def categorize_error(
        ...
        if status_code:
            category, is_retryable = self.status_code_categories.get(
                status_code, (ErrorCategory.UNKNOWN_ERROR, False)
            )
            ...
            # Special handling for server errors
                ...
                error_info.suggested_delay = self._calculate_server_error_delay(
                    status_code
                )
    ...
    def _categorize_exception(
        self, exception: Exception, url: Optional[str]
    ) -> ErrorInfo:
        ...
        if any(
            name in exception_name.lower()
            for name in ["connection", "network", "socket"]
        ):
            return ErrorInfo(
        ...
        # DNS exceptions
        if any(
            name in exception_name.lower() for name in ["dns", "resolve", "gaierror"]
        ):
            return ErrorInfo(
        ...
        # SSL/TLS exceptions
        if any(
            name in exception_name.lower() for name in ["ssl", "tls", "certificate"]
        ):
            return ErrorInfo(
    ...
    def _calculate_adaptive_delay(
        self, error_info: ErrorInfo, attempt: int, config: RetryConfig
    ) -> float:
    ...
    def record_error(self, url: str, error_info: ErrorInfo) -> None:
    ...
    def get_error_statistics(self, domain: Optional[str] = None) -> Dict[str, Any]:
        ...
        current_time = time.time()
        recent_errors = [
            timestamp for timestamp, _ in history if current_time - timestamp <= 3600
        ]
