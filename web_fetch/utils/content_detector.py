...
class ContentTypeDetector:
    ...
    def _detect_by_signature(
        self, content: bytes
    ) -> Tuple[Optional[ContentType], float]:
    ...
    def _detect_by_content_analysis(
        self, content: bytes
    ) -> Tuple[Optional[ContentType], float]:
    ...
    def _looks_like_csv(self, text: str) -> bool:
        ...
        return avg_commas >= 1 and all(
            abs(count - avg_commas) <= 2 for count in comma_counts
        )
    ...
    def _looks_like_html(self, text: str) -> bool:
        ...
        indicator_count = sum(
            1 for indicator in html_indicators if indicator in text_lower
        )
    ...
    def _looks_like_xml(self, text: str) -> bool:
        ...
        return len(tags) >= 2 and not any(
            tag.lower() in ["html", "head", "body"] for tag in tags
        )

    def _combine_detections(
        self, detections: List[Tuple[ContentType, float, str]]
    ) -> Tuple[ContentType, float]:
    ...
    def get_detection_info(
        ...
        methods = [
            ...
            (
                "mime",
                lambda: self._detect_by_mime_type(headers) if headers else (None, 0.0),
            ),
            ("url", lambda: self._detect_by_url_pattern(url) if url else (None, 0.0)),
            (
                "extension",
                lambda: (
                    self._detect_by_extension(filename) if filename else (None, 0.0)
                ),
            ),
        ...
        # Get final detection
        final_type, final_confidence = self.detect_content_type(
            content, url, headers, filename
        )
