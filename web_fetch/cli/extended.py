...
@click.pass_context
def rss(ctx: click.Context, url: str, max_items: int, include_content: bool):
    ...
    async def test_rss():
        ...
        try:
            result = await fetch_rss_feed(url, config=config, rss_config=rss_config)
...
def database(host: str, port: int, database: str, username: str, password: str, db_type: str, query: Optional[str], ssl: bool):
    """Test database connection with advanced options."""
    async def test_database():
        from ..convenience import fetch_database_query
        ...
        try:
            result = await fetch_database_query(db_config, test_query)
...
def storage(provider: str, bucket: str, access_key: str, secret_key: str, region: Optional[str], prefix: Optional[str]):
    """Test cloud storage connection."""
    async def test_storage():
        from ..convenience import fetch_cloud_storage
        from pydantic import SecretStr
...
@click.option('--include-content', is_flag=True, help='Include full content')
@click.pass_context
def rss(ctx: click.Context, url: str, max_items: int, output: Optional[str], output_format: str, include_content: bool):
    """Fetch RSS feed with advanced options."""
    async def fetch_rss_resource():
        from ..convenience import fetch_rss_feed
        import csv
...
def invalidate(tag: Optional[str], host: Optional[str], kind: Optional[str]):
