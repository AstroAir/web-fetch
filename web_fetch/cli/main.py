...
Examples:
  ...
  %(prog)s --crawler-status
    ...
    parser.add_argument(
        '--stream',
        action='store_true',
        help='Use streaming mode for downloads'
    )
    ...
    parser.add_argument(
        '--max-file-size',
        type=int,
        help='Maximum file size for downloads (bytes)'
    )
    ...
    parser.add_argument(
        '--validate-urls',
        action='store_true',
        help='Validate URLs before fetching'
    )

    parser.add_argument(
        '--normalize-urls',
        action='store_true',
        help='Normalize URLs before fetching'
    )
    ...
    parser.add_argument(
        '--crawler-status',
        action='store_true',
        help='Show crawler API status and exit'
    )
    ...
    if format_type == 'json':
        ...
        if hasattr(results, 'results'):  # BatchFetchResult
            ...
            for i, result in enumerate(results.results, 1):
                ...
                output_lines.append(f"{i:3d}. {status} {result.url} ({result.status_code}) {result.response_time:.2f}s")
                if result.error:
    ...
    try:
        ...
        if args.use_crawler:
            ...
            if args.crawler_operation == 'crawl':
                # Website crawling
                if len(urls) > 1:
                    print("Error: Crawl operation only supports single URL", file=sys.stderr)
                    sys.exit(1)
        ...
        # Handle streaming downloads
            ...
            if len(urls) > 1:
                print("Error: Streaming mode only supports single URL downloads", file=sys.stderr)
                sys.exit(1)
            ...
            async with streaming_fetcher:
                results = await streaming_fetcher.stream_fetch(request, progress_callback)
        ...
        # Handle caching
            ...
            results = await fetch_with_cache(urls[0], content_type, cache_config, config)
