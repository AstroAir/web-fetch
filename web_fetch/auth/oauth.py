...
class OAuth2Config(AuthConfig):
    ...
    authorization_url: Optional[HttpUrl] = Field(default=None, description="OAuth authorization endpoint URL")
    ...
    grant_type: str = Field(default=GrantType.CLIENT_CREDENTIALS, description="OAuth grant type")
    scope: Optional[str] = Field(default=None, description="OAuth scope")
    
    # For authorization code flow
    redirect_uri: Optional[str] = Field(default=None, description="Redirect URI for authorization code flow")
    authorization_code: Optional[str] = Field(default=None, description="Authorization code")
    ...
    # Token management
    refresh_token: Optional[str] = Field(default=None, description="Refresh token for token renewal")
    token_refresh_threshold: float = Field(default=300.0, description="Refresh token when expires within this many seconds")
...
class OAuth2Auth(AuthMethod):
    ...
    async def authenticate(self, **kwargs: Any) -> AuthResult:
        ...
        try:
            ...
            result = AuthResult(
                success=True,
                headers={"Authorization": f"{token_response.token_type} {token_response.access_token}"},
    ...
    async def _prepare_token_request(self, **kwargs: Any) -> Dict[str, str]:
        ...
        if self.config.grant_type == GrantType.AUTHORIZATION_CODE:
            if not self.config.authorization_code:
                raise AuthenticationError("Authorization code is required for authorization_code grant")
            ...
            if not self.config.username or not self.config.password:
                raise AuthenticationError("Username and password are required for password grant")
