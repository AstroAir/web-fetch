...
from .base import AuthConfig, AuthMethod, AuthResult, AuthType, AuthLocation
...
class APIKeyConfig(AuthConfig):
    ...
    prefix: Optional[str] = Field(default=None, description="Optional prefix for the API key (e.g., 'Bearer')")
    ...
    async def authenticate(self, **kwargs: Any) -> AuthResult:
        ...
        try:
            ...
            if self.config.location == AuthLocation.HEADER:
                result = AuthResult(
                    success=True,
                    headers={self.config.key_name: key_value}
                )
            elif self.config.location == AuthLocation.QUERY:
                result = AuthResult(
                    success=True,
                    params={self.config.key_name: key_value}
                )
            elif self.config.location == AuthLocation.BODY:
                result = AuthResult(
                    success=True,
                    body_data={self.config.key_name: key_value}
                )
            else:
                return AuthResult(
                    success=False,
