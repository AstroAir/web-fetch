...
class JWTConfig(AuthConfig):
    ...
    secret_key: Optional[str] = Field(default=None, description="Secret key for token signing")
    algorithm: str = Field(default="HS256", description="JWT signing algorithm")
    
    # Claims
    issuer: Optional[str] = Field(default=None, description="Token issuer (iss claim)")
    audience: Optional[str] = Field(default=None, description="Token audience (aud claim)")
    subject: Optional[str] = Field(default=None, description="Token subject (sub claim)")
...
class JWTAuth(AuthMethod):
    ...
    async def authenticate(self, **kwargs: Any) -> AuthResult:
        ...
        try:
            ...
            if self.config.token:
                ...
                return AuthResult(
                    success=False,
                    error="Either token or secret_key must be provided"
                )
    ...
    async def refresh(self) -> AuthResult:
        ...
        if not self.config.secret_key:
            return AuthResult(
                success=False,
                error="Cannot refresh JWT token without secret key"
            )
