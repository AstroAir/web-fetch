...
class AuthManager:
    ...
    def add_url_pattern(self, pattern: str, auth_method_name: str) -> None:
        ...
        if auth_method_name not in self._auth_methods:
            raise AuthenticationError(f"Authentication method '{auth_method_name}' not found")
    ...
    async def authenticate(self, method_name: Optional[str] = None, **kwargs: Any) -> AuthResult:
        """
        ...
        if method_name not in self._auth_methods:
            raise AuthenticationError(f"Authentication method '{method_name}' not found")
